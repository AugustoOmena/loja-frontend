...
import { useProcessingMessage } from "@/store/features/checkout/hooks/useProcessingMessage";
...
export const PixBoletoCheckout = () => {
  ...
  const [method, setMethod] = useState<"pix" | "boleto">(
    (location.state as { defaultMethod?: "pix" | "boleto" })?.defaultMethod ||
      "pix",
  );
  ...
  const [successData, setSuccessData] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const handlePay = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        setErrorModal({
          message:
            result.error || result.message || messages.paymentProcessingError,
              ...
              <div style={{ display: "flex", gap: 15 }}>
                <div style={{ width: "35%" }}>
                  ...
                  <input
                    style={{
                      ...styles.input,
                      borderColor: fieldErrors.zipCode
                        ? "#dc2626"
                        : colors.border,
                    }}
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <input
                    style={{
                      ...styles.input,
                      borderColor: fieldErrors.street
                        ? "#dc2626"
                        : colors.border,
                    }}
